...
for n_init_sample in [50]:
    for randomstate in seeds: 
        ...
        opt = BO4ML(search_space, fscore,conditional=con,hpo_prefix='name',isFair=isFair,
        ...
        ran_results.to_csv(HomeFolder+'/LOGS/BO4ML/'+Mclasstxt+'_'+acctxt+'_'+str(n_init_sample)+'_BO4ML_'+HPOalg+'_'
                               +dataset+'_'+str(randomstate)+'_'+str(dsfold)+'.csv', 
                               index = True, header=True)
        finallog= HomeFolder+"/TPE_BO4ML.csv"
        if (os.path.exists(finallog)==False):
            with open(finallog, "a") as f:    
                ...
                wr.writerow(['Mclasstxt','acctxt','dataname','HPOalg','random_state','initsample','mean',
                             'before','params','runtime','errorcount'])
        with open(finallog, "a") as f:
            wr = csv.writer(f, dialect='excel')
            wr.writerow([Mclasstxt,acctxt,dataset,dsfold,randomstate,n_init_sample,best,_bestBefore,params_best,runtime,errorcount])
